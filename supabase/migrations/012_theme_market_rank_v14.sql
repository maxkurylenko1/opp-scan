...
begin
  ...
  for r in
    with eligible as (
      select ot.*,tm.*,
             ...
             coalesce(
               ot.market_gap_score,
               least(10,5 + case when tm.exact_cluster_count>=3 then 1 else 0 end + case when tm.money_signal_count>=2 then 1 else 0 end + case when tm.independent_source_count>=2 then 2 else 0 end)::numeric
             ) as gap,
             case when ot.market_researched_at is null then 0::numeric
               else least(10,
                 2 + least(ot.market_competitor_count,3)*2 + least(ot.market_pricing_evidence_count,2)
               )::numeric end as market_confidence_bonus
  ...
  loop
    insert into public.opportunities(
      ...
      null,r.summary,
      format('%s exact clusters, %s signals, %s platform(s), %s money signals, confidence %s%%.%s',
        r.exact_cluster_count,r.signal_count,r.independent_source_count,r.money_signal_count,r.confidence,
        case when r.market_researched_at is not null then ' Competitor/pricing research verified.' else '' end),
    ...
    on conflict (theme_id) where theme_id is not null do update set
      ...
      pricing_hypothesis=excluded.pricing_hypothesis,status=excluded.status,
    ...
    insert into public.opportunity_score_breakdown(
      ...
      v_opp_id,r.avg_pain,r.avg_purchase_intent,r.reach,r.freq,r.recency,r.gap,r.buildability,r.recurring,r.confidence/10,0,r.score,'theme-v1.4'
    );
  ...
  if v_count>0 then
    ...
    values(v_week_start,v_week_end,now(),format('Generated %s ranked opportunity themes with market research aware scoring.',v_count),v_top,v_watch,jsonb_build_object('generated',v_count,'mode','theme-v1.4'))
