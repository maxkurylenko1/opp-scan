...
alter table public.experiments drop constraint if exists experiments_execution_state_check;
alter table public.experiments add constraint experiments_execution_state_check
  check (execution_state in ('not_started','active','completed','stopped'));

update public.experiments
set failure_contact_limit=coalesce(failure_contact_limit,target_sample_size,30)
where validation_version='validation-v1.5';
...
begin
  ...
  set execution_state=case
        when verdict in ('pass','fail') then 'completed'
        when v_contacted>0 then 'active'
  ...
  if e.verdict='pending' then
    if v_paid >= coalesce(e.success_paid_count,3)
       and v_delivered >= coalesce(e.success_delivered_count,2) then
      v_new_verdict:='pass';
    elsif v_contacted >= coalesce(e.failure_contact_limit,e.target_sample_size,30)
       and v_paid < coalesce(e.failure_paid_below_count,2) then
      v_new_verdict:='fail';
    end if;

    if v_new_verdict is not null then
      update public.experiments
      set verdict=v_new_verdict,ended_at=now(),execution_state='completed'
      where id=p_experiment_id;
  ...
  return jsonb_build_object(
    'prospects',v_prospects,'contacted',v_contacted,'replied',v_replied,
    'qualified',v_qualified,'paid',v_paid,'delivered',v_delivered,'lost',v_lost,
    'revenue',v_revenue,'verdict',coalesce(v_new_verdict,e.verdict)
  );
end;
$$;
...
create trigger validation_contacts_before_write before insert or update on public.validation_contacts
for each row execute function public.validation_contact_before_write();

drop trigger if exists validation_contacts_after_write on public.validation_contacts;
create trigger validation_contacts_after_write after insert or update or delete on public.validation_contacts
for each row execute function public.validation_contact_after_write();
