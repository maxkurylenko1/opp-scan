...
async function callPlanner(opportunity: any, competitors: any[]) {
  ...
  const schema = {
    ...
    properties: {
      ...
      channel: { type: "string" },
      offer: { type: "string" },
      offer_price: { type: ["number", "null"], minimum: 0 },
      offer_currency: { type: ["string", "null"] },
...
export async function generateValidationPlans(limit = 3, force = false) {
  ...
  for (const opportunity of selected) {
    ...
    const payload = {
      ...
      hypothesis: plan.hypothesis.slice(0, 2000),
      method: plan.method.slice(0, 4000),
      audience: plan.audience.slice(0, 2000),
      target_sample_size: plan.target_sample_size,
      success_metric: plan.success_metric.slice(0, 1000),
      ...
      outreach_message: plan.outreach_message.slice(0, 3000),
      followup_message: plan.followup_message.slice(0, 2000),
      notes: plan.notes.slice(0, 3000),
      verdict: "pending",
      generated_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };

    const { data: experiment, error: upsertError } = await supabase
      .from("experiments")
      .upsert(payload, { onConflict: "opportunity_id,validation_version" })
      .select("id")
      .single();
